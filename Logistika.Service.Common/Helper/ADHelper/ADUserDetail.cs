...
namespace Logistika.Service.Common.Helper.ADHelper
{
    public class ADUserDetail
    {
        ...
        private ADUserDetail(DirectoryEntry directoryUser)
        {
            ...
            var photo = GetProperty<byte[]>(directoryUser, ADProperties.THUMBNAILPHOTO);
            _photo = Convert.ToBase64String(photo, 0, (photo != null ? photo.Length : 0));
        ...
        private static T GetProperty<T>(DirectoryEntry userDetail, String propertyName)
        {
            if (userDetail.Properties.Contains(propertyName))
            {
                return (T)userDetail.Properties[propertyName][0];
            }
            return default(T);
        }
